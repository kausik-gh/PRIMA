"""Trigger lists for ContextFlag. This is the only module that may hold them."""
...
_CATEGORY_REGEX: dict[str, tuple[re.Pattern[str], ...]] = {
    "urgency": (re.compile(r"\bonly\s+\d+\s+minutes?\s+remain(?:s|ing)?\b"),),
    "secrecy": (re.compile(r"\bno\s+one\s+else\s+must\s+know\b"),),
    "fear": (re.compile(r"\bwarrant\s+has\s+been\s+issued\b"),),
    "greed": (re.compile(r"\bguaranteed\s+\d+\s*%\s+return\b"),),
    "bypass_approval": (re.compile(r"\bbypass\s+the\s+(?:otp|pin|2fa)\b"),),
}
...
def sample_text_for_categories(category_list: Sequence[str]) -> str:
    return " ".join(sample_text_for_category(name) for name in category_list)
...
def _category_hits(category: str, folded_text: str, tokens: tuple[str, ...]) -> bool:
