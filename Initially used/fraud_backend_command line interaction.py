...
def visualize_fraud_subgraph(G):
    ...
    if not fraud_nodes:
        print("No fraud nodes found.")
        return

    # Get 1-hop neighbors of fraud nodes
...
def simulate_coordinated_attack():
    ...
    if current_attack_index < len(attack_registry):
        ...
        return accounts_df, transactions_df, attack_time
...
final_dataset = build_multi_run_dataset(num_runs=30)
...
model = ml_detection(final_dataset)
current_attack_index = 0
...
print("\n=== Fraud Simulation Demo Mode ===")
print("Press Enter to simulate next attack.")
...
while True:
    ...
    accounts_df, transactions_df, attack_time = simulate_coordinated_attack()

    if accounts_df is not None:
        print("\nFraud Accounts (Ground Truth):")
        print(accounts_df[accounts_df["is_fraud"] == 1])

        # Filter only attack transactions
        attack_transactions = transactions_df[transactions_df["timestamp"] >= attack_time]
        G = build_transaction_graph(accounts_df, attack_transactions)
        visualize_fraud_subgraph(G)

        features_df = extract_node_features(accounts_df, attack_transactions, G)
        print("\nExtracted Features (Top 10 by Transaction Count):")
        print(features_df.sort_values("transaction_count", ascending=False).head())

        risk_df = rule_based_detection(features_df)
        print("\nRisk Scoring Results (Top 10):")
        print(risk_df.sort_values("risk_score", ascending=False).head(10))

        # -----------------------------
        # ML Detection Results
        # -----------------------------
        predictions = ml_predict(model, features_df)
        print("\nML Detection Results (Top 10 by Prediction Score):")
        print(predictions.sort_values("prediction_score", ascending=False).head(10))
